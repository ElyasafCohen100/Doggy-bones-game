<p align="center">
    <img src="me.jpg" width="140" style="border-radius:100%"/>
</p>

# 🐶 Doggy Bones Game 🦴

![GitHub stars](https://img.shields.io/github/stars/ElyasafCohen100/Doggy-bones-game?style=social)
![GitHub forks](https://img.shields.io/github/forks/ElyasafCohen100/Doggy-bones-game?style=social)

> **Mini Project | Creative Coding & Game Development**  
> A fun and interactive arcade-style game built with Processing

---

## 🏷️ Technologies & Tools

### 🎮 Game Development
![Java](https://img.shields.io/badge/Java-Core_Language-ED8B00?style=for-the-badge&logo=java&logoColor=white)
![Processing](https://img.shields.io/badge/Processing-Game_Engine-006699?style=for-the-badge)

### 🎨 Assets & Media
![Sprites](https://img.shields.io/badge/Sprites-Graphics-FF69B4?style=for-the-badge)
![Sound Effects](https://img.shields.io/badge/Sound-Effects-8A2BE2?style=for-the-badge)

---

## ✨ Overview ✨

**Doggy Bones Game** is a cute and fun arcade-style game built with **Processing** 🐶🦴  
You play as a little dog who loves collecting bones — but be careful!  
If you lose all your hearts, it’s game over 💔

The goal is simple: **collect as many bones as possible and reach a high score** 🎯  
Easy to play, fun to master!

---

## 🎮 How to Play 🎮

- ⌨️ Move the dog using the keyboard  
- 🦴 Catch the falling bones  
- ❤️ Avoid losing all your hearts  
- 🏆 Try to beat your high score  

---

## 📁 Project Files 📁

- `MyFirstApp.pde` – main game logic  
- `nitz13D.pde` – helper classes and extra logic  
- `data/` – images, sounds, and fonts  

---

## 🖼️ Screenshots 🖼️

### 🕹️ In-Game Gameplay  
![Gameplay](the_game.jpg)

### 🏆 Win Screen  
![You Win](you_win.jpg)

### 🎬 Game Over Screen  
![Game Over](game_over.jpg)

---

## 🎥 Gameplay Demo 🎥

![Doggy Bones Demo](doggy_bones_demo.gif)

---

## Create with good vibes by: 🎉

Elyasaf Cohen 🤟😎

[![Elyasaf Cohen](https://img.shields.io/badge/GitHub-ElyasafCohen100-0A66C2?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ElyasafCohen100)

---

## 💥 Final Notes 💥

This project demonstrates:
- Creative game design  
- Basic game mechanics and scoring systems  
- Use of graphics, sound, and user input  
- Fun-first development with clean structure  

---

> ✨ If you like this project – please leave a star! ✨ 
